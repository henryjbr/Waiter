...
let categoryPendingDeletionId = "";
let categoryRemovalPendingId = "";
let subcategoryPendingDeletionId = "";
let subcategoryEditorCategoryId = "";
...
async function submitCustomerOrder(event) {
  ...
  const order = {
    ...
    customerName: currentCustomerSession?.name || "",
...
function renderCategoryOverview() {
  ...
  if (!categories.some((category) => category.id === subcategoryEditorCategoryId)) {
    subcategoryEditorCategoryId = "";
  }
...
function categorySummaryCard(category) {
  ...
  const isSelected = subcategoryEditorCategoryId === category.id;
  ...
  title.title = `${category.subcategories.length} ${category.subcategories.length === 1 ? "subcategoria" : "subcategorias"} • ${itemCount} ${itemCount === 1 ? "item" : "itens"}`;

  controls.className = "category-summary-controls";
  removeButton.className = "category-remove-button";
  removeButton.type = "button";
  removeButton.textContent = "Excluir";
  removeButton.setAttribute("aria-label", `Excluir categoria ${category.name}`);
  removeButton.addEventListener("click", () => requestDeleteCategory(category.id));

  addButton.className = "category-add-button";
  addButton.type = "button";
  addButton.textContent = isSelected ? "−" : "+";
  addButton.setAttribute("aria-label", `${isSelected ? "Fechar cadastro de" : "Adicionar"} subcategoria em ${category.name}`);
  addButton.setAttribute("aria-expanded", String(isSelected));
  ...
  subcategoryList.className = "subcategory-row-list";
  if (category.subcategories.length) {
    category.subcategories.forEach((subcategory) => {
      ...
      deleteButton.className = "subcategory-delete-button";
      deleteButton.type = "button";
      deleteButton.textContent = "Excluir";
      deleteButton.setAttribute("aria-label", `Excluir subcategoria ${subcategory.name}`);
      deleteButton.addEventListener("click", () => requestDeleteSubcategory(category.id, subcategory.id));
      row.append(name, deleteButton);
      subcategoryList.appendChild(row);
    });
  } else {
    const empty = document.createElement("span");
    empty.className = "empty-subcategory-row";
    empty.textContent = "Nenhuma subcategoria cadastrada.";
    subcategoryList.appendChild(empty);
  }

  titleBlock.append(title);
  header.append(titleBlock, controls);
  card.append(header, subcategoryList);

  if (isSelected) {
    subcategoryForm.classList.remove("hidden");
    card.appendChild(subcategoryForm);
  }
...
function selectCategoryForSubcategory(categoryId) {
  subcategoryEditorCategoryId = categoryId;
...
function requestDeleteRestaurant(restaurantId) {
  ...
  categoryPendingDeletionId = "";
  categoryRemovalPendingId = "";
  subcategoryPendingDeletionId = "";
  restaurantPendingDeletionId = selected.id;
  confirmTitle.textContent = "Apagar restaurante?";
...
function requestDeleteClient(clientId) {
  ...
  confirmTitle.textContent = "Apagar cliente?";
...
function requestDeleteCategory(categoryId) {
  ...
  categoryPendingDeletionId = "";
  subcategoryPendingDeletionId = "";
  categoryRemovalPendingId = category.id;
  const itemCount = state.restaurant.items.filter((item) => item.categoryId === category.id).length;
  confirmTitle.textContent = "Excluir categoria?";
  confirmMessage.textContent = itemCount
    ? `A categoria "${category.name}" e ${itemCount} ${itemCount === 1 ? "item vinculado" : "itens vinculados"} serão excluídos.`
    : `A categoria "${category.name}" e suas subcategorias serão excluídas.`;
  confirmOverlay.classList.remove("hidden");
  confirmDeleteButton.focus();
}
...
function requestDeleteSubcategory(categoryId, subcategoryId) {
  ...
  subcategoryPendingDeletionId = subcategory.id;
  confirmTitle.textContent = "Excluir subcategoria?";
  confirmMessage.textContent = `A subcategoria "${subcategory.name}" será removida de "${category.name}".`;
  confirmOverlay.classList.remove("hidden");
  confirmDeleteButton.focus();
}
...
async function confirmDeleteRestaurant() {
  if (subcategoryPendingDeletionId) {
    await confirmDeleteSubcategory();
    return;
  }

  if (categoryRemovalPendingId) {
...
async function confirmDeleteSubcategory() {
  const category = getCategory(categoryPendingDeletionId);
  if (!category || !subcategoryPendingDeletionId) return;

  const deletedSubcategoryId = subcategoryPendingDeletionId;
  category.subcategories = category.subcategories.filter((subcategory) => subcategory.id !== deletedSubcategoryId);
  state.restaurant.items.forEach((item) => {
    if (item.categoryId === category.id && item.subcategoryId === deletedSubcategoryId) {
      item.subcategoryId = "";
    }
  });
...
async function confirmDeleteCategory() {
  if (!categoryRemovalPendingId) return;

  const deletedCategoryId = categoryRemovalPendingId;
  state.restaurant.categories = state.restaurant.categories.filter((category) => category.id !== deletedCategoryId);
  state.restaurant.items = state.restaurant.items.filter((item) => item.categoryId !== deletedCategoryId);
  if (subcategoryEditorCategoryId === deletedCategoryId) {
    subcategoryEditorCategoryId = "";
  }
